...
exports.generateMandiriVa = async function generateMandiriVa(
	setupConfiguration,
	paymentCodeRequest
) {
...
exports.generateDOKUVa = async function generateDOKUVa(
	setupConfiguration,
	paymentCodeRequest
) {
...
exports.generateBSIVa = async function generateBSIVa(
	setupConfiguration,
	paymentCodeRequest
) {
	setupConfiguration.api_target = "/bsm-virtual-account/v2/payment-code";
	return await post(setupConfiguration, paymentCodeRequest);
};
...
function post(setupConfiguration, paymentCodeRequest) {
	...
	setupConfiguration.request_id = Math.floor(
		Math.random() * Math.floor(100000)
	);
	setupConfiguration.request_timestamp =
	...
	return axios
		.post(
			setupConfiguration.serverLocation + setupConfiguration.api_target,
			paymentCodeRequest,
			axiosConfig
		)
		.then((res) => {
	...
	let response = {
		...
		virtual_account_info: {
			virtual_account_number:
				request["virtual_account_info"]["virtual_account_number"],
		},
	};
	return response;
};
...
exports.getSignature = function getSignature(header, notifyRequest, key) {
	...
	return new Promise((resolve, reject) => {
		try {
			var bodySha256 = CryptoJS.enc.Base64.stringify(
				CryptoJS.SHA256(notifyRequest)
			);
			...
			var signature = CryptoJS.enc.Base64.stringify(
				CryptoJS.HmacSHA256(signatureComponents, key)
			);
...
function createSignature(setupConfiguration, incomingRequest) {
	...
	var bodySha256 = CryptoJS.enc.Base64.stringify(
		CryptoJS.SHA256(JSON.stringify(incomingRequest))
	);
	var signatureComponents =
		"Client-Id:" +
		setupConfiguration.client_id +
		"\n" +
		"Request-Id:" +
		setupConfiguration.request_id +
		"\n" +
		"Request-Timestamp:" +
		setupConfiguration.request_timestamp +
		"\n" +
		"Request-Target:" +
		setupConfiguration.api_target +
		"\n" +
		"Digest:" +
		bodySha256;
	var signature = CryptoJS.enc.Base64.stringify(
		CryptoJS.HmacSHA256(signatureComponents, setupConfiguration.shared_key)
	);
